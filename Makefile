...
ifeq ($(PLATFORM),macos)
	...
	@if command -v brew >/dev/null 2>&1; then \
		brew install cmake openssl@3 pkg-config jsoncpp libcurl; \
	else \
	...
	sudo apt-get update
	sudo apt-get install -y build-essential cmake libssl-dev libjsoncpp-dev libcurl4-openssl-dev pkg-config
	# For RPM-based systems
	# sudo yum install -y gcc-c++ cmake openssl-devel jsoncpp-devel libcurl-devel
else ifeq ($(PLATFORM),windows)
...
docker-run:
	docker run -d --name $(PROJECT_NAME)-$(VERSION) -p 587:587 $(PROJECT_NAME):$(VERSION)
...
ifeq ($(PLATFORM),macos)
	...
	@if exist scripts\build-windows.bat ( \
		...
		echo "  sc create SimpleSMTPMailer binPath= \"$(INSTALL_PREFIX)\\bin\\$(PROJECT_NAME).exe\""; \
		echo "  sc start SimpleSMTPMailer"; \
	)
endif
...
ifeq ($(PLATFORM),macos)
	...
	@echo "Step 2: Creating DMG with PKG, LICENSE, README, and docs..."
	@DMG_TEMP_DIR="/tmp/$(PROJECT_NAME)-dmg-$$$$"; \
	rm -rf "$$DMG_TEMP_DIR" 2>/dev/null || true; \
	mkdir -p "$$DMG_TEMP_DIR"; \
	echo "  Created temporary directory: $$DMG_TEMP_DIR"; \
	PKG_FILE=$$(ls $(DIST_DIR)/*.pkg 2>/dev/null | head -1); \
	if [ -f "$$PKG_FILE" ]; then \
		cp "$$PKG_FILE" "$$DMG_TEMP_DIR/"; \
		echo "  Added PKG installer: $$(basename $$PKG_FILE)"; \
	fi; \
	if [ -f README.md ]; then \
		cp README.md "$$DMG_TEMP_DIR/" 2>/dev/null || true; \
		echo "  Added README.md"; \
	fi; \
	if [ -f LICENSE ]; then \
		cp LICENSE "$$DMG_TEMP_DIR/" 2>/dev/null || true; \
		echo "  Added LICENSE"; \
	fi; \
	if [ -d docs ]; then \
		cp -r docs "$$DMG_TEMP_DIR/" 2>/dev/null || true; \
		echo "  Added docs directory"; \
	fi; \
	DMG_NAME="$(PROJECT_NAME)-$(VERSION)-$(OS_TYPE)-$(ARCH)"; \
	DMG_FILE="$(DIST_DIR)/$$DMG_NAME.dmg"; \
	echo "  Creating DMG: $$DMG_FILE"; \
	if hdiutil create -volname "$(PROJECT_NAME)" -srcfolder "$$DMG_TEMP_DIR" -ov -format UDBZ "$$DMG_FILE" 2>&1; then \
		echo "DMG package created: $$DMG_FILE"; \
		ls -lh "$$DMG_FILE"; \
		rm -rf "$$DMG_TEMP_DIR" 2>/dev/null || true; \
	else \
...
ifeq ($(PLATFORM),macos)
	...
	@sc config SimpleSMTPMailer start= auto
endif
...
ifeq ($(PLATFORM),macos)
	...
	@sc config SimpleSMTPMailer start= disabled
endif
...
ifeq ($(PLATFORM),macos)
	...
	@sc query SimpleSMTPMailer >nul 2>&1 && ( \
		sc stop SimpleSMTPMailer; \
		sc delete SimpleSMTPMailer; \
		echo "Service uninstalled successfully"; \
	) || echo "Service not found"
endif
...
help:
	@echo "High-performance SMTP mailer with queue management and OAuth2 support - Main Help"
	@echo "=================================================================================="
	@echo ""
...
help-all:
	@echo "High-performance SMTP mailer with queue management and OAuth2 support - All Available Targets"
	@echo "=============================================================================================="
	@echo ""
	@echo "Essential targets:"
	@echo "  all              - Build the project (default)"
